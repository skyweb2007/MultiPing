...
uses
  ...
  Graphics, Dialogs, ExtCtrls, StdCtrls, ComCtrls, ActnList, IniFiles;
...
type
  ...
  private
    ...
    FCharHeight: integer;
    FLineHeight: integer;
    procedure LogPingStatus(ip: string; status: boolean);
...
begin
  pb := TPaintBox(Sender);
  if FPingIpCount < 0 then PaintEmpty(pb)
...
begin
  ...
  begin
    if (GetTickCount64 - FStartTick) mod 1000 = 0 then UpdateStatus;
...
procedure TFormMain.LogPingStatus(ip: string; status: boolean);
begin
  ip := Copy(ip, Length(FIpSegment) + 2, 3);
  FStatus[StrToInt(ip)] := status;
  inc(FPingIpCount);
end;
...
begin
  ...
  try
    FColor1 := ini.ReadInteger('Colors', 'PingOK', clFuchsia);
    FColor2 := ini.ReadInteger('Colors', 'PingFailed', clMenu);
    FColor3 := ini.ReadInteger('Colors', 'Background', clWhite);
